...
class Recognizer:
    ...
    def create_inputs(self, imgs, img_info):
        inputs = {}
        img_shape = []
        scale_factor = []
        ...
        if len(imgs) == 1:
            ...
            inputs["img_shape"] = np.array(
                    (img_info[0]["img_shape"], )).astype("float32")
            inputs["scale_factor"] = np.array(
                    (img_info[0]["scale_factor"], )).astype("float32")
            return inputs
        
        img_shape = np.array([info["img_shape"] for info in img_info], dtype="float32")
        scale_factor = np.array([info["scale_factor"] for info in img_info], dtype="float32")

        inputs["img_shape"] = np.concatenate(img_shape, axis=0)
        ...
        for img in imgs:
            ...
            padding_img = np.zeros(
                (img_c, max_shape_h, max_shape_w), dtype=np.float32)
            padding_img[:, :img_h, :img_w] = img
            padding_imgs.append(padding_img)
    ...
    def preprocess(self, image_list):
        ...
        if "scale_factor" in self.input_names:
            preprocess_ops = []
            for op_info in [
                {"interp": 2, "keep_ratio": False, "target_size": [800, 608], "type": "LinearRezise"},
            ...
            for img_path in image_list:
                img, img_info = preprocess(img_path, preprocess_ops)
                inputs.append({"image": np.array((img,)).astype("float32"),
