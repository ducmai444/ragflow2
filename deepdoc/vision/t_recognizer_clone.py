...
def main(args):
    ...
    for i, lyt in enumerate(layouts):
        ...
        logging.info("Save resutl to: " + outputs[i])
...
def get_table_html(img, tb_cpns, ocr):
    ...
    def gather(kwd, fzy=10, ption=0.6):
        ...
        eles = LayoutRecognizer.sort_Y_firstly(
            [r for r in tb_cpns if re.match(kwd, r["label"])]
        )
    ...
    clmns = sorted([r for r in tb_cpns if re.match(
        r"table column$", r["label"]
    )], key=lambda x: x["x0"])
    clmns = LayoutRecognizer.layouts_cleanup(boxes, clmns, 0.5)
